...
bool compareCaseInsensitive(const string& s1, const string& s2) {
	return((s1.size() == s2.size()) &&
			equal(s1.begin(), s1.end(), s2.begin(), cmp));
}
...
void circuit::read_iccad2015_file(const char* input)
{
	...
	read_lef(dot_lef);
	read_verilog(dot_verilog, dot_sdc);
	read_sdc(dot_sdc);
...
void circuit::read_verilog(const string &input, const string &src_sdc)
{
	ifstream dot_sdc(src_sdc.c_str());
	if (!dot_sdc.good())
	{
		cerr << "read_verilog:: cannot open `" << src_sdc << "' for reading." << endl;
		exit(1);
	}
	bool valid_clock = read_clock(dot_sdc, clock_name, clock_port, clock_period);
#ifdef DEBUG
	...
	do
	{
		...
		if (valid)
		{
			...
			for (unsigned i = 0; i < pinNetPairs.size(); ++i)
			{
				...
				myPin->isFlopCkPort = (myPin->isFlopInput && compareCaseInsensitive(pinNetPairs[i].first, FFClkPortName));

				// NOTE: in this contest, all LCBs are INV_Z80 and connected to iccad_clk
				//if(cellType == "INV_Z80" && myNet->name == "iccad_clk")
				if(cellType == "INV_Z80" && myNet->name == clock_port)
					myCell->isLCB = true;
...
void circuit::read_lef_macro_pin(ifstream &is, macro* myMacro)
{
	...
	if(compareCaseInsensitive(pinName, FFClkPortName))
		myMacro->isFlop = true;
}
...
void circuit::convert_pl_to_def(const string final_def, const string src_pl)
{
	ofstream dot_def((final_def).c_str());
	if(!dot_def.good())
		return;
	...
	/* parsing .pl file */
	ifstream dot_pl((src_pl).c_str());
	if(!dot_pl.good())
	{
		cout << src_pl << " does not exist. exiting .." <<endl;
		exit(1);
	}
