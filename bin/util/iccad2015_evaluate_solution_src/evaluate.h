...
struct net
{
	...
	net() : name(""), source(numeric_limits<unsigned>::max()), lumped_cap(0.0) {}
	void print();
};
...
class circuit
{
	private:
		...
		void write_spef(const string &output);
		void write_timing(const string &output);

        // Jinwook Jung, 170820
        void write_ops(const string &output);
	...
	public:
		...
		void read_verilog(const string &input, const string &src_sdc);
		...
		void convert_pl_to_def(const string final_def, const string src_pl);
