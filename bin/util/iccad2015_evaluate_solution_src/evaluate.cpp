...
bool circuit::measure_timing()
{
    ...
    if (!DISABLE_INPUT_DRIVERS) {
        for(vector<unsigned>::iterator PI=PIs.begin() ; PI!=PIs.end() ; ++PI)
        {
            ...
            string in;
            string out;
            if (!findFirstPin(theDriverMac, "INPUT", in)) exit(1);
            if (!findFirstPin(theDriverMac, "OUTPUT", out)) exit(1);

            timer.set_slew(driverName+":"+in, EARLY, RISE, thePI->rTran);
            timer.set_slew(driverName+":"+in, EARLY, FALL, thePI->fTran);
            timer.set_slew(driverName+":"+in, LATE, RISE, thePI->rTran);
            timer.set_slew(driverName+":"+in, LATE, FALL, thePI->fTran);
            timer.set_at(driverName+":"+in, EARLY, RISE, 0.0);
            timer.set_at(driverName+":"+in, EARLY, FALL, 0.0);
            timer.set_at(driverName+":"+in, LATE, RISE, 0.0);
            timer.set_at(driverName+":"+in, LATE, FALL, 0.0);

            // #####################################################################
            inPins.insert(driverName+":"+in);
            driverIns.insert(driverName+":"+out);		
            // #####################################################################
        }
    } // end if
    // 3. read out results
    ...
    for(vector<pin>::iterator thePin=pins.begin() ; thePin!=pins.end() ; ++thePin)
    {
        ...
        {
            //continue;
            macro *theDriverMac = &macros[ thePin->driverType ];

            string out;
            if (!findFirstPin(theDriverMac, "OUTPUT", out)) exit(1);

            thePin->earlySlk=min(timer.report_slack(thePin->name+"_drv:"+out, EARLY, RISE), timer.report_slack(thePin->name+"_drv:"+out, EARLY, FALL)); 
            thePin->lateSlk =min(timer.report_slack(thePin->name+"_drv:"+out, LATE,  RISE), timer.report_slack(thePin->name+"_drv:"+out, LATE,  FALL)); 
    ...
    const bool opt_hide_input_drivers = !true;
    ...
    if(!opt_show_cell_core)
        for (auto &nd : *alnd) {
            ...
            if (opt_hide_input_drivers && 
                    (driverIns.count(pn->name()) || inPins.count(pn->name())) ) {
                cout << "skiped input driver related pin: " << pn->name()<<endl;
                continue;
            } // end if
            ...
            if(!opt_hide_input_drivers && driverIns.count(pn->name()) > 0) {
                string str = pn->name();
            ...
            if("FE_OFC147646_n191095:o" == pn->name() || "FE_OFC147646_n191095:ck" == pn->name() ){
                cout<<pn->name()<<"\n";
                cout<<"slack: "<< nd->slack(EARLY, FALL) << " "
                    << nd->slack(EARLY, RISE) << " "
                    << nd->slack(LATE, FALL) << " "
                    << nd->slack(LATE, RISE);
            ...
            if(opt_show_time){
                ...
                if (opt_show_wdelay) {
                    ...
                    out << " wdelay: "
                        << (nd->at(EARLY, FALL) - nodeRoot->at(EARLY, FALL)) << " "
                        << (nd->at(EARLY, RISE) - nodeRoot->at(EARLY, RISE)) << " "
                        << (nd->at(LATE, FALL) - nodeRoot->at(LATE, FALL)) << " "
...
void circuit::build_steiner()
{
    ...
    {
        if (theNet->source == numeric_limits<unsigned>::max() || theNet->sinks.size() == 0) {
            continue;
        }
...
void circuit::slice_longwires(unsigned threshold)
{
    ...
    for(vector<net>::iterator theNet=nets.begin() ; theNet != nets.end() ; ++theNet)
    {
        ...
        for(unsigned i=0 ; i<orig_wire_seg_cnt ; i++)
        {
            ...
            if(theSeg->second  > threshold && ENABLE_SLICING)
            {
                ...
                string prev_pin_name=start_pin_name+"_1";
                theSeg->first.second=prev_pin_name;
                theSeg->second=sliced_wirelength;
                for(unsigned j=1 ; j<slice_cnt ; j++, tot_slice_cnt++)
                {
                    string cur_pin_name=start_pin_name+"_"+to_string(static_cast<long long unsigned>(j+1));
...
void circuit::write_spef(const string &output)
{
    ofstream dot_spef(output.c_str());
    if(!dot_spef.good())
    {
        cerr << "write_spef:: cannot open `" << output << "' for writing." << endl;
        exit(1);
    }
    stringstream feed;
    feed.precision(5);
    ...
    feed<< "*DESIGN_FLOW PIN_CAP NONE" << endl;
    feed<< "*DIVIDER /" <<endl;
    feed<< "*DELIMITER :" <<endl;
    ...
    for(vector<net>::iterator theNet = nets.begin() ; theNet != nets.end() ; ++theNet)
    {
        ...
        if(theNet->name == clock_port )//|| theNet->name == "in2_9_")
            continue;
...
/**
 * Added by Jinwook Jung to support iTimerC2.0.1.
 *
 * @param output Base file name.
 */
void circuit::write_ops (const string &output)
{
    string late_ops = output + ".late.ops";
    string early_ops = output + ".early.ops";

    ofstream ofs(late_ops.c_str());
    if(!ofs.good())
    {
        cerr << "write_ops:: cannot open `" << output << "' for writing." << endl;
        exit(1);
    }

    // report slack -pin <pin name> [-rise|-fall] [-early|-late]
    // report worst paths [-pin <pin name>] [-numPaths <int>]
    for (auto pin =pins.begin(); pin != pins.end(); ++pin) {
        // collect slacks @ timing end points
        if(pin->type == PO_PIN || (pin->isFlopInput && !pin->isFlopCkPort))
        {
            ofs << "report_slack -pin " << pin->name << " -rise -late" << endl;
            ofs << "report_slack -pin " << pin->name << " -fall -late" << endl;
        }
    }
    ofs << "report_worst_paths" << endl;
    ofs << "report_wns" << endl;
    ofs << "report_tns" << endl;
    ofs.close();

    ofs.open(early_ops.c_str());
    if(!ofs.good())
    {
        cerr << "write_ops:: cannot open `" << output << "' for writing." << endl;
        exit(1);
    ...
    for (auto pin =pins.begin(); pin != pins.end(); ++pin) {
        ...
        if(pin->type == PO_PIN || (pin->isFlopInput && !pin->isFlopCkPort))
        {
            ofs << "report_slack -pin " << pin->name << " -rise -early" << endl;
            ofs << "report_slack -pin " << pin->name << " -fall -early" << endl;
        }
    }
    ofs.close();
}
